"""
Input: Pandas dataframe. If the parameter 'labeled' is not explicitly passed as False, the function assumes that
the first column contains the correct classes or the assigned clusters.
Example: |Class|Width|Length|Height|...| or |Assigned_Cluster|Width|Length|Height|...|

Output: A figure containing a 3D plot corresponding to the dimension reduced dataframe. The dimensionality reduction
is done using PCA.
"""
...
def dataframe_to_pca_plot(df, labeled=True):
    ...
    fig = px.scatter_3d(transformed_df, x='x', y='y', z='z', color='cluster', width=800, height=800)
